/**
 * All credits reserved to @Dev4Mod with his amazing repo https://github.com/Dev4Mod/WaEnhancer
 */
...
public class ReflectionUtils {
    ...
    public static Method findMethodUsingFilter(Class<?> clazz, Predicate<Method> predicate) {
        do {
            Optional<Method> results = Arrays.stream(clazz.getDeclaredMethods()).filter(predicate).findFirst();
    ...
    public static Method[] findAllMethodsUsingFilter(Class<?> clazz, Predicate<Method> predicate) {
        do {
            List<Method> results = Arrays.stream(clazz.getDeclaredMethods()).filter(predicate).collect(Collectors.toList());
    ...
    public static Field findFieldUsingFilter(Class<?> clazz, Predicate<Field> predicate) {
        do {
            Optional<Field> results = Arrays.stream(clazz.getDeclaredFields()).filter(predicate).findFirst();
            if (results.isPresent()) return results.get();
        } while ((clazz = clazz.getSuperclass()) != null);
    ...
    public static Constructor<?>[] findAllConstructorsUsingFilter(Class<?> clazz, Predicate<Constructor<?>> predicate) {
        do {
            List<Constructor<?>> results = Arrays.stream(clazz.getDeclaredConstructors()).filter(predicate).collect(Collectors.toList());
    ...
    public static Constructor<?> findConstructorUsingFilter(Class<?> clazz, Predicate<Constructor<?>> predicate) {
        do {
            Optional<Constructor<?>> results = Arrays.stream(clazz.getDeclaredConstructors()).filter(predicate).findFirst();
            if (results.isPresent()) return results.get();
        } while ((clazz = clazz.getSuperclass()) != null);
        throw new RuntimeException("Field not found");
    }
    ...
    public static Field[] findAllFieldsUsingFilter(Class<?> clazz, @NonNull Predicate<Field> predicate) {
        do {
            List<Field> results = Arrays.stream(clazz.getDeclaredFields()).filter(predicate).collect(Collectors.toList());
    ...
    public static Object callMethod(Method method, Object instance, Object... args) {
        try {
            ...
            if (count != args.length) {
                Object[] newargs = initArray(method.getParameterTypes());
    ...
    public static List<Pair<Integer, Object>> findArrayOfType(Object[] args, Class<?> type) {
        ArrayList<Pair<Integer, Object>> result = new ArrayList<Pair<Integer, Object>>();
        for (int i = 0; i < args.length; i++) {
            Object arg = args[i];
    ...
    public static boolean isCalledFromString(String contains) {
        ...
        String text = Arrays.toString(trace);
    ...
    public static <T> T getArg(Object[] args, Class<T> typeClass, int i) {
        List<Pair<Integer, Object>> list = findArrayOfType(args, typeClass);
    ...
    // ── Helper tambahan untuk aplikasi mod ──────────────────

    private static Context sAppContext;

    /**
     * Dapatkan application context, diinisialisasi saat on_load().
     */
